...
コンテキストに載っている情報量が増えるほど回答精度は落ちる。
コンテキストに残すのは「今まさに必要な情報」だけにし、以下を使い分けること：
- 今は不要だが後で使いそうな情報は `ai-stash/` に書き出してコンテキストから外す
- 大きな調査結果やツール出力はファイルに保存し、要約だけをコンテキストに残す
- 情報取得時は grep で部分取得、ファイル編集前は全文取得等、状況に応じて最適な方法を選択する

## ai-stash 運用

`ai-stash/` はAIが管理するコンテキスト退避用ストレージ（git管理外）。
...
→ ai-stash に書き込む・整理する際は `instructions/stash-guide.md` の手順に従うこと

## セッション管理

以下の兆候が出たらセッションを切り替える：
- 大きなテーマが切り替わるとき
- 応答精度が落ちてきたとき（存在しないファイルの参照、同じ質問への回答ブレ等）
- コンテキスト圧縮が発動したとき

→ 切り替え時は `instructions/session-switch.md` の手順に従うこと

## エージェント手順書

詳細な手順が必要な場合は `instructions/` 配下を参照すること。

- `instructions/stash-guide.md` — ai-stash への書き込み・整理を行うとき
- `instructions/stash-check.md` — ユーザーから「スタッシュチェック」と依頼があったとき
- `instructions/session-switch.md` — セッションを切り替えるとき
