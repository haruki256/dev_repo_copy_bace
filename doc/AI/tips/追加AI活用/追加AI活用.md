...
├── instructions/ ← エージェント向け手順書（オンデマンド読み込み）
│   ├── stash-check.md ← ai-stash 棚卸し手順
│   └── ... ← 今後追加可能
├── ai-stash/ ← AI 退避ストレージ（git 管理外・個人ごと）
...
AI の退避ストレージ（ai-stash）は `xx-service/ai-stash/` に配置する。`xx/` 配下ではなく xx-service ルートに置く理由：
- **git 管理不要**：ai-stash は個人の作業文脈であり、push する必要がない。xx-service ルートは git 管理外なので `.gitignore` の設定も不要
...
退避ファイルを「今の案件で頻繁に参照するもの（hot）」と「安定した情報でたまに参照するもの（cold）」に分離する。
...
棚卸し手順の詳細は `instructions/stash-check.md` に記載。AGENTS.md に「スタッシュチェックと依頼があったら instructions/stash-check.md を読んで実行」と指示しておくことで、ユーザーが「スタッシュチェックして」と言うだけで実行される。

### instructions/ ディレクトリ

ai-stash/（知識データ）とは別に、エージェント向けの手順書を置くディレクトリ。
- AGENTS.md はポインタだけ持ち、詳細手順は instructions/ に置く
...
- 詳細情報は ai-stash/ や instructions/ に退避 → 必要時のみ読み込み

## コンテキストファイルの例

親フォルダに配置する AGENTS.md / CLAUDE.md のサンプルは [`例/`](例/) 配下を参照。

- [AGENTS.md](例/AGENTS.md) — ワークスペースルートに置く AGENTS.md のサンプル
- [CLAUDE.md](例/CLAUDE.md) — AGENTS.md へのポインタとして置く CLAUDE.md のサンプル
- [stash-check.md](例/memory-check.md) — スタッシュチェック手順書のサンプル（instructions/ に配置）
