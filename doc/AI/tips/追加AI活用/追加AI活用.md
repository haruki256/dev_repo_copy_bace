...
│   ├── memory-check.md ← メモリ棚卸し手順
│   └── ... ← 今後追加可能
├── memory/ ← AI メモリ（git 管理外・個人ごと）
│   ├── MEMORY.md ← ポインタインデックス（常時参照）
│   ├── hot/ ← 今の案件で頻繁に参照
│   └── cold/ ← 安定した情報、たまに参照
├── xx/ ← メインリポジトリ
...
- **プロジェクトのルール・規約**は `xx/AGENTS.md` に集約されている。xx-service の AGENTS.md はそこへの誘導に加えて、ワークスペース固有の設定（メモリ運用、サブエージェント、プリフェッチ、セッション管理等）を持つ
- **作業対象のリポジトリはユーザーの指示に従う**。特に指定がなければ xx が対象
- AI に複数リポジトリを見せると **コンテキストが広がる** ため、作業対象を明確に指示すること

### メモリの配置

AI のメモリ（3層メモリ構造の第1層・第2層）は `xx-service/memory/` に配置する。`xx/` 配下ではなく xx-service ルートに置く理由：
- **git 管理不要**：メモリは個人の作業文脈であり、push する必要がない。xx-service ルートは git 管理外なので `.gitignore` の設定も不要
- **個人ごとに最適化**：同じプロジェクトでも担当領域や作業文脈は人によって異なる。メモリは各自が育てるもの
- **リポジトリ横断**：メモリには複数リポジトリにまたがる知識も含まれるため、特定リポジトリの配下に置くより親フォルダに置く方が自然
...
## 機能追加・実装時に参照
- アーキテクチャ判断 → memory/architecture.md
- コーディング規約 → memory/conventions.md

## バグ修正・調査時に参照
- 既知の問題 → memory/known-issues.md
- 過去のバグパターン → memory/bug-patterns.md

## 設計・方針検討時に参照
- 技術選定の経緯 → memory/dependencies.md
- 案件ごとの設計判断 → memory/案件名-design.md
```

### hot/cold 分離

メモリファイルを「今の案件で頻繁に参照するもの（hot）」と「安定した情報でたまに参照するもの（cold）」に分離する。
...
棚卸し手順の詳細は `instructions/memory-check.md` に記載。AGENTS.md に「メモリーチェックと依頼があったら instructions/memory-check.md を読んで実行」と指示しておくことで、ユーザーが「メモリーチェックして」と言うだけで実行される。

### instructions/ ディレクトリ

memory/（知識データ）とは別に、エージェント向けの手順書を置くディレクトリ。
...
- 詳細情報は memory/ や instructions/ に退避 → 必要時のみ読み込み
...
- [memory-check.md](例/memory-check.md) — メモリーチェック手順書のサンプル（instructions/ に配置）
