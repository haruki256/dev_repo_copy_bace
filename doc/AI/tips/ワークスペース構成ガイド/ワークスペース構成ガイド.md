...
ai-stash を中継点にしてセッションを切り替える。AIの自動圧縮に頼らず、ユーザー主導で切る（理由・タイミングは `AI協働開発ガイド.md` の「セッション管理」を参照）。
...
現セッション                    新セッション
    ...
    └─ セッション終了               │
                                   ├─ CLAUDE.md 読み込み
                                   ├─ STASH_INDEX.md 確認
                                   ├─ hot/ から状態・結論・次にやること・関連ファイルを読む
                                   ├─ 必要に応じて関連 cold/ を確認
                                   └─ 作業再開
```

hot/ ファイルに「関連ファイル」と「関連 cold/」のポインタを含めることで、新セッションのAIは hot/ だけ読めば必要な参照先がすべてわかる。cold/ を直接探し回る必要がない。

フォーマットの詳細は `例/ai-instructions/stash-guide.md` を参照。
